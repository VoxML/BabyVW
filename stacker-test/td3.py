...
def main():
    parser = argparse.ArgumentParser(description=textwrap.dedent('''
        Stacker parameters.
        Example usage:
        \tTrain new model for 500 timesteps using vector observations only, and save the model in the "cube_stacking_model" directory: "python td3.py -b stacker -l cube_stacking_model -t 500 --train --vector_obs"
        \tTest td3_1 for 50 timesteps: "python td3.py -b stacker -m td3_1 -t 50 --test"
        \tLoad td3_1, continue training for 100 timesteps, and save the result as "td3_2": "python ddpg.py -b stacker -m td3_1 -M td3_2" -t 100 --train"
    ...
    if train:
        if new_model_name is None:
            if visual_obs and vector_obs:
                model = TD3("MultiInputPolicy", env, learning_rate=1e-4, action_noise=action_noise, verbose=1, tensorboard_log="./" + tb_name + "/")
            elif visual_obs:
                model = TD3("CnnPolicy", env, learning_rate=1e-4, action_noise=action_noise, verbose=1, tensorboard_log="./" + tb_name + "/")
            elif vector_obs:
