...
def main():
    parser = argparse.ArgumentParser(description=textwrap.dedent('''
        Stacker parameters.
        Example usage:
        \tTrain new model for 500 timesteps using vector observations only, and save the model in the "cube_stacking_model" directory: "python sac.py -b stacker -l cube_stacking_model -t 500 --train --vector_obs"
        \tTest sac_1 for 50 timesteps: "python sac.py -b stacker -m sac_1 -t 50 --test"
        \tLoad sac_1, continue training for 100 timesteps, and save the result as "sac_2": "python a2c.py -b stacker -m sac_1 -M sac_2" -t 100 --train"
    ...
    if train:
        if new_model_name is None:
            if visual_obs and vector_obs:
                model = SAC("MultiInputPolicy", env, learning_rate=1e-4, verbose=1, tensorboard_log="./" + tb_name + "/")
            elif visual_obs:
                model = SAC("CnnPolicy", env, learning_rate=1e-4, verbose=1, tensorboard_log="./" + tb_name + "/")
            elif vector_obs:
                model = SAC("MlpPolicy", env, learning_rate=1e-4, verbose=1, tensorboard_log="./" + tb_name + "/")
        else:
            model = SAC.load(log_dir + "/" + model_name, env)
    ...
    if test:
        model = SAC.load(log_dir + "/" + model_name)
        print(model)
        print(model.policy)
